...
def preprocessed_data(transformed_data: pd.DataFrame):
    u_unique = transformed_data.user_id.unique()
    user2Idx = {o:i+1 for i,o in enumerate(u_unique)}
    m_unique = transformed_data.movie_id.unique()
    movie2Idx = {o:i+1 for i,o in enumerate(m_unique)}
    transformed_data['encoded_user_id'] = transformed_data.user_id.apply(lambda x: user2Idx[x])
    transformed_data['encoded_movie_id'] = transformed_data.movie_id.apply(lambda x: movie2Idx[x])
    
    preprocessed_training_data = transformed_data.copy()
